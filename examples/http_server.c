...
void process_http_client(void *arg)
{
	...
	while (rn_http_request_get(&http)) {
		rn_log("Request uri=%.*s thread=%d", rn_buffer_size(&http.request.uri), rn_buffer_ptr(&http.request.uri), rn_sched_self()->id);
...
void process_http_server(void *arg)
{
	...
	rn_log("Thread %d started.", rn_sched_self()->id);
	while ((client = rn_tcp_accept(socket, NULL, NULL)) != NULL) {
		rn_task_start(rn_sched_self(), process_http_client, client);
	}
	rn_socket_destroy(socket);
	rn_log("Thread %d stopped.", rn_sched_self()->id);
}

void signal_handler(int unused(signal))
{
	rn_log("Exiting...");
	rn_sched_stop(master);
}
