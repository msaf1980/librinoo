...
int main()
{
	...
	for (i = 0; i < 500; i++) {
		XTEST(rn_vector_remove(&vector, (uint32_t) i) == 0);
	}
	XTEST(rn_vector_size(&vector) == 500);
	for (i = 0; i < 500; i++) {
		XTEST(rn_vector_get(&vector, (uint32_t) i) == INT_TO_PTR(i * 2 + 1));
	}
