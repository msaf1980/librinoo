 ...
 */
t_rinoosocket *rinoo_tcp(t_rinoosched *sched, t_rinoosocket_run run_func)
{
	return rinoo_socket(sched, AF_INET, SOCK_STREAM, run_func);
}
 ...
 */
t_rinoosocket *rinoo_tcp_accept(t_rinoosocket *socket, t_rinoosocket_run run_func, u32 timeout, t_ip *fromip, u32 *fromport)
{
	t_rinoosocket	*new;
	struct sockaddr_in addr;

#warning timeout unused
	new = rinoo_socket_accept(socket, (struct sockaddr *) &addr, (socklen_t *)(int[]){(sizeof(struct sockaddr))}, run_func);
	if (fromip != NULL) {
